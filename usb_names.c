	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		cp = buf;
                if (buf[0] == 'P' && buf[1] == 'H' && buf[2] == 'Y' && buf[3] == 'S' && buf[4] == 'D' &&
                    buf[5] == 'E' && buf[6] == 'S' && /*isspace(buf[7])*/ buf[7] == ' ') {
                        ...
                        while (isspace(*cp))
                                cp++;
                        if (!isxdigit(*cp)) {
                                DEBUG( "Invalid Physdes type at line %u\n", linectr);
                                continue;
                        }
                        u = strtoul(cp, &cp, 16);
                        while (isspace(*cp))
                        ...
                        if (new_physdes(cp, u))
                                DEBUG( "Duplicate Physdes  type spec at line %u terminal type %04x %s\n", linectr, u, cp);
                ...
                if (buf[0] == 'P' && buf[1] == 'H' && buf[2] == 'Y' && /*isspace(buf[3])*/ buf[3] == ' ') {
                        ...
                        while (isspace(*cp))
                                cp++;
                        if (!isxdigit(*cp)) {
                                DEBUG( "Invalid PHY type at line %u\n", linectr);
                                continue;
                        }
                        u = strtoul(cp, &cp, 16);
                        while (isspace(*cp))
                        ...
                        if (new_physdes(cp, u))
                                DEBUG( "Duplicate PHY type spec at line %u terminal type %04x %s\n", linectr, u, cp);
                ...
                if (buf[0] == 'B' && buf[1] == 'I' && buf[2] == 'A' && buf[3] == 'S' && /*isspace(buf[4])*/ buf[4] == ' ') {
                        ...
                        while (isspace(*cp))
                                cp++;
                        if (!isxdigit(*cp)) {
                                DEBUG( "Invalid BIAS type at line %u\n", linectr);
                                continue;
                        }
                        u = strtoul(cp, &cp, 16);
                        while (isspace(*cp))
                        ...
                        if (new_bias(cp, u))
                                DEBUG( "Duplicate BIAS  type spec at line %u terminal type %04x %s\n", linectr, u, cp);
                ...
                if (buf[0] == 'L' && /*isspace(buf[1])*/ buf[1] == ' ') {
                        ...
                        while (isspace(*cp))
                                cp++;
                        if (!isxdigit(*cp)) {
                                DEBUG( "Invalid LANGID spec at line %u\n", linectr);
                                continue;
                        }
                        u = strtoul(cp, &cp, 16);
                        while (isspace(*cp))
                        ...
                        if (new_langid(cp, u))
                                DEBUG( "Duplicate LANGID spec at line %u language-id %04x %s\n", linectr, u, cp);
		...
		if (buf[0] == 'C' && /*isspace(buf[1])*/ buf[1] == ' ') {
			...
			while (isspace(*cp))
				cp++;
			if (!isxdigit(*cp)) {
				DEBUG( "Invalid class spec at line %u\n", linectr);
				continue;
			}
			u = strtoul(cp, &cp, 16);
			while (isspace(*cp))
			...
			if (new_class(cp, u))
				DEBUG( "Duplicate class spec at line %u class %04x %s\n", linectr, u, cp);
		...
		if (buf[0] == 'A' && buf[1] == 'T' && isspace(buf[2])) {
			...
			while (isspace(*cp))
				cp++;
			if (!isxdigit(*cp)) {
				DEBUG( "Invalid audio terminal type at line %u\n", linectr);
				continue;
			}
			u = strtoul(cp, &cp, 16);
			while (isspace(*cp))
			...
			if (new_audioterminal(cp, u))
				DEBUG( "Duplicate audio terminal type spec at line %u terminal type %04x %s\n", linectr, u, cp);
		...
		if (isxdigit(*cp)) {
			...
			while (isspace(*cp))
				cp++;
			if (!*cp) {
				DEBUG( "Invalid vendor spec at line %u\n", linectr);
				continue;
			}
			if (new_vendor(cp, u))
				DEBUG( "Duplicate vendor spec at line %u vendor %04x %s\n", linectr, u, cp);
		...
		if (buf[0] == '\t' && isxdigit(buf[1])) {
			...
			while (isspace(*cp))
				cp++;
			if (!*cp) {
				DEBUG( "Invalid product/subclass spec at line %u\n", linectr);
				continue;
			}
			if (lastvendor != -1) {
				if (new_product(cp, lastvendor, u))
					DEBUG( "Duplicate product spec at line %u product %04x:%04x %s\n", linectr, lastvendor, u, cp);
				DBG(printf("line %5u product %04x:%04x %s\n", linectr, lastvendor, u, cp));
				continue;
			}
			if (lastclass != -1) {
				if (new_subclass(cp, lastclass, u))
					DEBUG( "Duplicate subclass spec at line %u class %02x:%02x %s\n", linectr, lastclass, u, cp);
			...
			if (lasthut != -1) {
				if (new_hutus(cp, (lasthut << 16)+u))
					DEBUG( "Duplicate HUT Usage Spec at line %u\n", linectr);
				continue;
			}
			if (lastlang != -1) {
                                if (new_langid(cp, lastlang+(u<<10)))
                                        DEBUG( "Duplicate LANGID Usage Spec at line %u\n", linectr);
                                continue;
                        }
			DEBUG( "Product/Subclass spec without prior Vendor/Class spec at line %u\n", linectr);
			continue;
		}
		if (buf[0] == '\t' && buf[1] == '\t' && isxdigit(buf[2])) {
			...
			while (isspace(*cp))
				cp++;
			if (!*cp) {
				DEBUG( "Invalid protocol spec at line %u\n", linectr);
				continue;
			}
			if (lastclass != -1 && lastsubclass != -1) {
				if (new_protocol(cp, lastclass, lastsubclass, u))
					DEBUG( "Duplicate protocol spec at line %u class %02x:%02x:%02x %s\n", linectr, lastclass, lastsubclass, u, cp);
				DBG(printf("line %5u protocol %02x:%02x:%02x %s\n", linectr, lastclass, lastsubclass, u, cp));
				continue;
			}
			DEBUG( "Protocol spec without prior Class and Subclass spec at line %u\n", linectr);
			continue;
		}
		if (buf[0] == 'H' && buf[1] == 'I' && buf[2] == 'D' && /*isspace(buf[3])*/ buf[3] == ' ') {
			cp = buf + 4;
                        while (isspace(*cp))
                                cp++;
                        if (!isxdigit(*cp)) {
                                DEBUG( "Invalid HID type at line %u\n", linectr);
                                continue;
                        }
                        u = strtoul(cp, &cp, 16);
                        while (isspace(*cp))
                        ...
                        if (new_hid(cp, u))
                                DEBUG( "Duplicate HID type spec at line %u terminal type %04x %s\n", linectr, u, cp);
                ...
                if (buf[0] == 'H' && buf[1] == 'U' && buf[2] == 'T' && /*isspace(buf[3])*/ buf[3] == ' ') {
                        ...
                        while (isspace(*cp))
                                cp++;
                        if (!isxdigit(*cp)) {
                                DEBUG( "Invalid HUT type at line %u\n", linectr);
                                continue;
                        }
                        u = strtoul(cp, &cp, 16);
                        while (isspace(*cp))
                        ...
                        if (new_huts(cp, u))
                                DEBUG( "Duplicate HUT type spec at line %u terminal type %04x %s\n", linectr, u, cp);
			...
			lasthut = u;
                ...
                if (buf[0] == 'R' && buf[1] == ' ') {
                        ...
                        while (isspace(*cp))
                                cp++;
                        if (!isxdigit(*cp)) {
                                DEBUG( "Invalid Report type at line %u\n", linectr);
                                continue;
                        }
                        u = strtoul(cp, &cp, 16);
                        while (isspace(*cp))
                        ...
                        if (new_reporttag(cp, u))
                                DEBUG( "Duplicate Report type spec at line %u terminal type %04x %s\n", linectr, u, cp);
                        DBG(printf("line %5u Report type %02x %s\n", linectr, u, cp));
                        continue;

                }			
		DEBUG( "Unknown line at line %u\n", linectr);
	}
}
